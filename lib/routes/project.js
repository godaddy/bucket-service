...
/**
 * @swagger
 * /projects/{name}/appArea:
 *   post:
 *     tags:
 *       - Projects
 *     description: Add a new appArea for the project.
 *     produces:
 *       - application/json
 *     parameters:
 *       - name: name
 *         description: Name of the project you are adding appArea to
 *         in: path
 *         required: true
 *         type: string
 *         example: "Barracuda"
 *       - name: appArea
 *         description: New appArea to be added to project.appAreas
 *         in: body
 *         schema:
 *           type: object
 *           properties:
 ...
 *        description: Internal server error occurred. Please create an issue on github repo.
 */
function addAppArea({ params, body }, res, callback) {
  ProjectModel.find({ name: params.name }, (err, projects) => {
    if (err) {
      res.statusCode = 500;
      res.json({
        error: err
    ...
    const project = projects[0];
    const { appAreas } = project;
    if (body.appArea)
      appAreas.push(body.appArea);
    project.appAreas = _.uniq(appAreas).sort();
...
function register(app) {
  ...
  app.post('/projects/:name/appArea', addAppArea);
