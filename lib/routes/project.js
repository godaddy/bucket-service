...
function updateOne({ params, body }, res, callback) {
  ...
  ProjectModel.find({ name: findName }, (findErr, projects) => {
    if (findErr) {
      ...
      if (body.hasOwnProperty('name') && newName) project.name = newName;
      if (body.hasOwnProperty('description') && description) project.description = description;
      if (body.hasOwnProperty('appAreas') && appAreas) project.appAreas = _.compact(appAreas);
